...
class appointments extends database {
    ...
    /**
     * On crée un methode qui va calculer le nombre de ligne de la table appointments
     * @return ARRAY contenant la valeur du nombre de ligne dans le champ totalRows
     */
    public function GetNumberTotalRows() {
        // on crée un requête pour calculer le nombre total de ligne de la table appointments
        $query = 'SELECT COUNT(`id`) AS `totalRows` FROM `appointments`';
    ...
    /**
     * On crée un methode qui va effectuer une recherche de patients dans la table appointments
     * @return ARRAY
     */
    public function findAppointmentBySearch($search) {
        // On met notre requète dans la variable $query qui selectionne tous les champs de la table appointments
    ...
    public function GetSomeAppointments($start, $limit) {
        ...
        $query = 'SELECT DATE_FORMAT(`appointments`.`dateHour`, "%d/%m/%Y") AS `date`,
                    ...
                    LEFT JOIN `patients`
                    ON `appointments`.`idPatients` = `patients`.`id`
                    ORDER BY `appointments`.`dateHour` LIMIT :limit OFFSET :start';
        // On crée un objet $result qui exécute la méthode query() avec comme paramètre $query
        $result = $this->dataBase->prepare($query);
